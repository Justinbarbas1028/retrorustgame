...
impl Game for SnakeGame {
    ...
    fn draw(&self, frame: &mut Frame, area: Rect, palette: &ThemePalette) {
        ...
        for x in 0..GRID_WIDTH as usize {
            visual_grid[0][x] = Some(Style::default().fg(palette.muted));
            visual_grid[GRID_HEIGHT as usize - 1][x] = Some(Style::default().fg(palette.muted));
        }
        for y in 0..GRID_HEIGHT as usize {
            visual_grid[y][0] = Some(Style::default().fg(palette.muted));
            visual_grid[y][GRID_WIDTH as usize - 1] = Some(Style::default().fg(palette.muted));
        }

        // Draw Food
        visual_grid[self.food.1 as usize][self.food.0 as usize] = Some(Style::default().fg(palette.danger).add_modifier(Modifier::BOLD));
        ...
        for (i, &pos) in self.snake.iter().enumerate() {
            if pos.0 >= 0 && pos.0 < GRID_WIDTH && pos.1 >= 0 && pos.1 < GRID_HEIGHT {
                let color = if i == 0 { palette.accent } else { palette.accent };
        ...
        let stats_content = vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled(" SPEED ", Style::default().fg(palette.accent)),
                Span::styled(format!("  {} mph", speed_pct + 10), Style::default().fg(palette.body)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled(" LENGTH", Style::default().fg(palette.accent)),
                Span::styled(format!("  {:03}", self.snake.len()), Style::default().fg(palette.body)),
            ]),
        ];
        ...
        if self.paused {
            ...
            let message = vec![
                Line::from(""),
                Line::from(Span::styled(" CRASH DEATH! ", Style::default().fg(palette.danger).add_modifier(Modifier::BOLD))),
