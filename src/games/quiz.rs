...
impl QuizGame {
    ...
    fn next_question(&mut self) {
        ...
        let questions = vec![
            ("What is the capital of France?", 2, "1. Berlin  2. Paris  3. Rome"),
            ("Which planet is closest to the Sun?", 3, "1. Earth  2. Mars  3. Mercury"),
            ("What is 7 * 8?", 2, "1. 54  2. 56  3. 64"),
            ("Who wrote Hamlet?", 1, "1. Shakespeare  2. Dickens  3. Orwell"),
        ];
        let idx = rng.gen_range(0..questions.len());
        let q = &questions[idx];
        self.target = q.1;
        self.prompt = format!("Question: {}\nOptions: {}", q.0, q.2);
