...
impl ZorkGame {
    ...
    fn get_rooms(&self) -> Vec<Room> {
        vec![
            Room { name: "West of House", desc: "You are standing in an open field west of a white house. There is a mailbox here.", choices: vec!["Open Mailbox", "Go North", "Go East"] },
            Room { name: "North of House", desc: "You are facing the north side of the house. The windows are boarded up.", choices: vec!["Go South", "Examine Boarded Windows", "Go West"] },
            Room { name: "Inside Mailbox", desc: "You find a leaflet describing the legendary Zork cave.", choices: vec!["Take Leaflet", "Close Mailbox", "Read Leaflet"] }
        ]
    }
    ...
    fn handle_choice(&mut self, choice_idx: usize) {
        ...
        match self.current_room {
            0 => match choice_idx {
                0 => { self.current_room = 2; self.log.push("Opened the mailbox.".to_string()); }
                1 => { self.current_room = 1; self.log.push("Moved North.".to_string()); }
                _ => { self.log.push("The front door is locked.".to_string()); }
            },
            1 => match choice_idx {
                0 => { self.current_room = 0; self.log.push("Returned West.".to_string()); }
                1 => { self.log.push("No way to open them without a crowbar.".to_string()); }
                _ => { self.current_room = 0; self.log.push("Went back West.".to_string()); }
            },
            2 => match choice_idx {
                0 => {
                    if !self.inventory.contains(&"Leaflet".to_string()) {
                        self.inventory.push("Leaflet".to_string());
                        self.log.push("Obtained Leaflet!".to_string());
                        self.score += 150;
                    } else {
                        self.log.push("Already taken.".to_string());
                    }
                }
                1 => { self.current_room = 0; self.log.push("Returned to West of House.".to_string()); }
                _ => {
                    if self.inventory.contains(&"Leaflet".to_string()) {
                        self.score += 500;
                        self.log.push("You read the leaflet and win the adventure!".to_string());
                        self.game_over = true;
                    } else {
                        self.log.push("Take the leaflet first.".to_string());
                    }
                }
            },
            _ => {}
...
impl Default for ZorkGame {
    fn default() -> Self {
        Self::new()
    }
}

impl Game for ZorkGame {
