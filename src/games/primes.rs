...
impl PrimesGame {
    ...
    fn next_question(&mut self) {
        ...
        let composite = vec![4, 6, 8, 9, 10, 12, 14, 15, 16, 18, 20, 21, 22, 24];
        let is_prime = rng.gen_bool(0.5);
        if is_prime {
            let p = primes[rng.gen_range(0..primes.len())];
            self.target = 1;
            self.prompt = format!("Is {} prime? 1=YES, 2=NO", p);
        } else {
            let c = composite[rng.gen_range(0..composite.len())];
            self.target = 2;
            self.prompt = format!("Is {} prime? 1=YES, 2=NO", c);
        }
...
impl Default for PrimesGame {
    fn default() -> Self {
        Self::new()
    }
}

impl Game for PrimesGame {
